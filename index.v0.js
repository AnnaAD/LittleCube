...
function splashScreen() {
    ...
    doRect(buttonPlay.left, buttonPlay.top, buttonPlay.right - buttonPlay.left, buttonPlay.bottom - buttonPlay.top, '#00FFFF', 0, '');
    ...
    doRect(buttonOptions.left, buttonOptions.top, buttonOptions.right - buttonOptions.left, buttonOptions.bottom - buttonOptions.top, '#00FFFF', 0, '');
    ...
    doRect(buttonInts.left, buttonInts.top, buttonInts.right - buttonInts.left, buttonInts.bottom - buttonInts.top, '#00FFFF', 0, '');
...
function makeLevel() {
    ...
    boxes.push({x: 0, y: 0,                   width: 1,            height: WINDOW_HEIGHT, mode: 0});
...
function update() {
    ...
    for (var i = 0; i < boxes.length; i++) {
        doRect(boxes[i].x, boxes[i].y, boxes[i].width, boxes[i].height, cmap(boxes[i].mode), 0, '');
...
function drawBackground() {
    doRect(0, 0, WINDOW_WIDTH, WINDOW_HEIGHT, 'gray', 0, '');
}
...
function drawPlayer() {
    // body
    doRect(player.x, player.y, player.width, player.height, 'yellow', 0, '');
	...
	// closed eyes
	doRect(player.x + player.width/2.0 - 3.5 - 3, player.y + 2, 6, 6, 'yellow', 0.5, 'black');
	doRect(player.x + player.width/2.0 + 3.5 - 3, player.y + 2, 6, 6, 'yellow', 0.5, 'black');
    } else {
	blinkCnt--;

	// open eyes
	doRect(player.x + player.width/2.0 - 3.5 - 3, player.y + 2, 6, 6, '#eeeeee', 0.5, 'black');
	doRect(player.x + player.width/2.0 + 3.5 - 3, player.y + 2, 6, 6, '#eeeeee', 0.5, 'black');
	...
	doRect(player.x + player.width/2.0 - 3.5 - 1 + dx, player.y + 4, 2, 2, '#3366ff', 0, '');
	doRect(player.x + player.width/2.0 + 3.5 - 1 + dx, player.y + 4, 2, 2, '#3366ff', 0, '');
    }
	...
	doRect(player.x + 4,  player.y + 13, 8, 1, 'black', 0, '');
	doRect(player.x + 4,  player.y + 11, 1, 2, 'black', 0, '');
	doRect(player.x + 11, player.y + 11, 1, 2, 'black', 0, '');
    }
